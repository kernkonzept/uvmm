...
public:
  ...
  {
    warn().printf("========= Dumping VMCS state ============\n");
    warn().printf("(C) VPID: 0x%llx\n", vmx_read(L4VCPU_VMCS_VPID));
    warn().printf("(C) Int notification vector: 0x%llx\n", vmx_read(L4VCPU_VMCS_PIR_NOTIFICATION_VECTOR));
    warn().printf("(C) EPTP index: 0x%llx\n", vmx_read(L4VCPU_VMCS_EPTP_INDEX));
    warn().printf("(C) EPT pointer: 0x%llx\n", vmx_read(L4VCPU_VMCS_EPT_POINTER));
    warn().printf("(C) Pin-based execution control: 0x%llx\n",
                  vmx_read(L4VCPU_VMCS_PIN_BASED_VM_EXEC_CTLS));
    warn().printf("(C) Primary execution control: 0x%llx\n",
                  vmx_read(L4VCPU_VMCS_PRI_PROC_BASED_VM_EXEC_CTLS));
    warn().printf("(C) Secondary execution control: 0x%llx\n",
                  vmx_read(L4VCPU_VMCS_SEC_PROC_BASED_VM_EXEC_CTLS));
    void *ext = (void *)((char *)_vmcs - 0x400);
    warn().printf("(c) basic capabilities: 0x%llx\n",
                  l4_vm_vmx_get_caps(ext, L4_VM_VMX_BASIC_REG));
    warn().printf("(C) Real pin-based execution control: 0x%llx\n",
                  l4_vm_vmx_get_caps(ext, L4_VM_VMX_TRUE_PINBASED_CTLS_REG));
    warn().printf("(C) Real primary execution control: 0x%llx\n",
                  l4_vm_vmx_get_caps(ext, L4_VM_VMX_TRUE_PROCBASED_CTLS_REG));
    warn().printf("(C) Real secondary execution control: 0x%llx\n",
                  l4_vm_vmx_get_caps(ext, L4_VM_VMX_PROCBASED_CTLS2_REG));

    warn().printf("(G) ES selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_ES_SELECTOR));
    warn().printf("(G) CS selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_CS_SELECTOR));
    warn().printf("(G) SS selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_SS_SELECTOR));
    warn().printf("(G) DS selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_DS_SELECTOR));
    warn().printf("(G) FS selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_FS_SELECTOR));
    warn().printf("(G) GS selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_GS_SELECTOR));
    warn().printf("(G) GDTR base: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_GDTR_BASE));
    warn().printf("(G) IDTR base: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_IDTR_BASE));

    warn().printf("(G) LDTR selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_LDTR_SELECTOR));
    warn().printf("(G) TR selector: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_TR_SELECTOR));
    warn().printf("(G) interrupt status: 0x%llx\n", vmx_read(L4VCPU_VMCS_GUEST_INTERRUPT_STATUS));

    warn().printf("(C) IO bitmap A: 0x%llx\n", vmx_read(L4VCPU_VMCS_ADDRESS_IO_BITMAP_A));
    warn().printf("(C) IO bitmap B: 0x%llx\n", vmx_read(L4VCPU_VMCS_ADDRESS_IO_BITMAP_B));
    warn().printf("(C) MSR bitmaps: 0x%llx\n", vmx_read(L4VCPU_VMCS_ADDRESS_MSR_BITMAP));
    warn().printf("(C) Exit MSR store address: 0x%llx\n",
                  vmx_read(L4VCPU_VMCS_VM_EXIT_MSR_STORE_ADDRESS));
    warn().printf("(C) Exit MSR load address: 0x%llx\n",
